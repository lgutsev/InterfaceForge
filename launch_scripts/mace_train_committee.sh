...
#
# Submit the three new seeds as separate Slurm jobs:
...
# Prevent two simultaneously running jobs from writing to the same seed.
...
# All committee members use exactly the same fixed train/validation/test split.
...
# Support both current and older MACE names for the second training stage.
STAGE_TWO_ARGS=()
...
if [[ -x "$MACE_EVAL_BIN" ]]; then
    ...
    # Prefer the uncompiled second-stage model, then the uncompiled base model.
    for candidate in \
        "$MODEL_DIR/${MODEL_NAME}_stagetwo.model" \
        "$MODEL_DIR/${MODEL_NAME}.model"; do
        if [[ -f "$candidate" ]]; then
    ...
    # Compatibility fallback for MACE versions with different final naming.
    ...
    if [[ -n "$MODEL_PATH" && -f "$MODEL_PATH" ]]; then
        ...
        echo "Check $MODEL_DIR and $CHECKPOINTS_DIR manually."
    fi
else
