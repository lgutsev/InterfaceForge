...
| MLIP comparison | matched-frame MACE vs DPA-2 accuracy audit on identical canonical test configurations: frame-identity proof, committee micro/macro/grouped metrics, committee-spread calibration | **Automated-test verified.** `validate_membership`, the force-metric definitions, a synthetic `prepare → status → finalize` run, and five `SafetyError` guards pass; no real MACE/DPA-2 committee has been compared, and MACE inference is presently forced to float32 while training uses float64. |
