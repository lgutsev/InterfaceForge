...
| Slab-referenced separation energy, DFT vs MLIP (`iface validate separation-energy`) | Unverified/code-only | Computes γ_sep = (E(slab_a)+E(slab_b)−E(interface))/(n·A) in J/m² for hand-built interface/half-slab sets: DFT read from each sub-directory's OUTCAR, and one or more MACE/DeePMD committees evaluated in place via ASE. Synthetic tests cover the excess/area arithmetic, committee ensemble/spread/Δ-vs-DFT, the pending-run and missing-directory paths, the literature overlay, and report+figure generation; the MACE/DeePMD ASE calculators themselves are exercised only through mocked backends. No real DFT+committee comparison has been run through it. `--reference free-surface` equals the Dupré work of adhesion only if the supplied half-slabs are genuinely relaxed free surfaces (not checked); `--reference bulk` is recorded but its scaling is not implemented. See `docs/separation-energy.md`. |
