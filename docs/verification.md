...
| Matched MACE vs DPA-2 comparison audit (`iface mlip-compare`) | Automated-test only | `validate_membership`, the centered/vector force-metric definitions, and the generated MACE evaluator's syntax have unit coverage; a synthetic `prepare -> status -> finalize` exercise and five `SafetyError` guards (frame-identity drift, model arity, missing manifest, incomplete finalize, corrupted `dp test` reference column) pass. No real MACE/DPA-2 committee has been run through the workflow, the cross-backend numbers have not been checked against the standalone MACE test table or DeePMD `rmse_overall.csv`, and MACE inference is currently forced to float32 while the committee trains at float64. See `docs/mlip-comparison.md`. |
