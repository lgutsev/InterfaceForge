...
| Bulk-referenced interfacial energy (`iface validate interface-energy`) | Automated-test only | Computes γ_int(T) in J/m² from the canonical dataset by referencing each unoxidized interface leaf to its same-temperature bulk phases, with gcd-based formula-unit matching, nitrogen-balance checks, longest-axis area, and block-average error bars. A synthetic-dataset test verifies the excess arithmetic, the unit conversion, missing-reference handling, and the oxidized-leaf skip. It is a potential-energy excess, not a rigorous free energy, and has not been checked against a literature TiN/SiN interfacial energy. See `docs/interface-energy.md`. |
