...
| Work-of-adhesion preparation and audit | Unverified/code-only | Preparation is ported from a script the maintainer already used successfully outside InterfaceForge; the audit step reuses the existing, unit-tested `iface audit`/`iface validate` math. Both have automated coverage, but no adhesion campaign has been run end to end (prepare -> VASP -> audit) through this integration. The optional literature comparison (`-c campaign.yaml`, matching `validation.references` / bundled `reference_profiles` against the computed J/m²) has unit coverage for the matching and delta arithmetic only. |
