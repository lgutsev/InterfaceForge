...
| DeePMD DPA-2 training, checkpoint continuation, freezing, and evaluation | Human-tested | A real PyTorch DPA-2 committee was trained on LONI from the synchronized NPY dataset, continued from checkpoints, frozen, and evaluated across the canonical test systems with `dp test`. This status is specific to the exercised DPA-2/runtime path; DPA-1/3/4, DPA-2/DPA-3 fine-tuning (`dpa2_ft`/`dpa3_ft` — generated `--finetune ... --model-branch ... --use-pretrain-script` dispatch and the flat/keyed `finetune` block have automated coverage only), cross-version model loading and LAMMPS deployment remain separate gates. |
...
| Work-of-adhesion preparation, audit, and summary | Unverified/code-only | Preparation is ported from a script the maintainer already used successfully outside InterfaceForge; the audit step reuses the existing, unit-tested `iface audit`/`iface validate` math. Both have automated coverage, but no adhesion campaign has been run end to end (prepare -> VASP -> audit) through this integration. Covered by synthetic tests only: the optional literature comparison (`-c campaign.yaml`, matching `validation.references` / bundled `reference_profiles`); `--interface-sp` (fresh interface single-point, preferred over the reference OUTCAR, and the W_sep-vs-W_ad label switch from `slab_mode`); and the `iface vasp adhesion summary` roll-up — per-interface re-audit, grouping the same leaf's DFT and MLFF trees onto one row with `delta_vs_dft`, short-label disambiguation, pending handling, CSV/markdown/JSON, and the two-panel (W / MLIP−DFT) publication figure. |
...
| Slab-referenced separation energy, DFT vs MLIP (`iface validate separation-energy`) | Unverified/code-only | Computes γ_sep = (E(slab_a)+E(slab_b)−E(interface))/(n·A) in J/m² for hand-built interface/half-slab sets: DFT read from each sub-directory's OUTCAR, and one or more MACE/DeePMD committees evaluated in place via ASE. Synthetic tests cover the excess/area arithmetic, committee ensemble/spread/Δ-vs-DFT, pending and missing paths, literature overlays, report+figure generation, adhesion-tree discovery, unique same-named DeePMD members, and validated merging of MACE/DeePMD JSON partials produced in isolated environments. The MACE/DeePMD ASE calculators themselves are exercised only through mocked backends; no real DFT+committee comparison has been run through it. `--reference free-surface` equals the Dupré work of adhesion only if the supplied half-slabs are genuinely relaxed free surfaces (not checked); `--reference bulk` is recorded but its scaling is not implemented. See `docs/separation-energy.md`. |
...
| Committee archiving and Hugging Face packaging (`iface committee collect --engine deepmd`, `iface package`) | Automated-test only | `collect_committee` now also bundles DeePMD frozen-model committees (`model_NNN/` discovery, frozen-model requirement, seed recovery from `ensemble_manifest.json`, duplicate/missing-member guards). `iface package dataset-archive` / `iface collect --archive` write one checksummed ZIP of a canonical dataset; `iface package huggingface` renders a model card with YAML frontmatter, `.gitattributes`, provenance manifest and `UPLOAD.md` from a verified bundle. `iface package verify` recomputes every checksum. `--dedupe` stores the DeePMD NPY tree only (`iface collect` now also writes `move_mask.npy` / `system_meta.json` per system so it is a complete record) and `iface package materialize` regenerates `*.extxyz` from it with an exact-round-trip float writer (not ASE's ~8-decimal default), self-verifying every frame's position/force/cell/energy/constraint-mask against the source arrays before publishing. `pack_dataset_archive` also recognizes the `iface-mapped-collect` / leaf-heritage dataset layout (`leaf_manifest.json` at the root plus one under `deepmd/`, e.g. the periodic SiN/TiN/TiO example), mirror-archiving it the same way `iface collect` datasets are (`--dedupe` still requires the `iface collect` sidecars and refuses cleanly otherwise). `iface package campaign` runs collect + package for every committee a campaign has (by the `mace_committee_520eV/{mace_committee,mace_finetune_committee}` and `models/deepmd/<arch>` conventions `iface mlip-progress` / `iface train` already use) plus the dataset archive in one call, catching and reporting a per-component failure (e.g. an existing bundle name on a re-run) rather than aborting the rest. Synthetic tests cover discovery, rejection paths, archive round-trips, tamper detection, card frontmatter, fine-tune `base_model`, metric embedding, the dedupe precision round-trip (bit-exact float64, independently checked outside the tool's own self-verification), missing-sidecar rejection/degradation, overwrite protection, the leaf-heritage dataset flavor, and the campaign-wide sweep (full success, missing pieces reported as skipped not fatal, and a same-tag re-run reporting immutability errors without touching the first run's output). No real Hugging Face upload, and no archive→restore→retrain cycle through actual MACE/DeePMD training, has been run; the packager never contacts the Hub. |
